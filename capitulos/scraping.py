...
for i in range(400, 481):
  ...
  with open(f'{i}.html', 'w', encoding="utf-8") as arquivo:
      arquivo.write(f'<style>{style1}</style><body style="background-color: #222222; color: white; margin: 25px;">{titulo}<h2 style="text-align: center;">{chaptername}</h2><p style="text-align: center;">Lançamento {data}</p>{noticia}</body>')
